...
public class UploadFileUtil {
	 ...
	 * @param files		要保存的文件
	 * @param picProjectFile : 图片服务器的位置   tomcat/webapps/uploadPdfs/
	 * @param  picProjectUrl  : 图片服务器的url   http://localhost:8080/uploadPdfs/
	 * @param  picRootName	:图片项目在服务器中的webapps下的名字 uploadPdfs
	 * @return
	 */
	public static Map<String, UploadFile> uploadFile(HttpServletRequest request, List<MultipartFile> files, String picRootName ) {
		...
		if (files != null && files.size() > 0) {
			//request.getSession().getServletContext().getRealPath("/")  =>D:\Tomcat\apache-tomcat-7.0.30-windows-x86\apache-tomcat-7.0.30\webapps\DouBanZx
			//getParentFile()   =>D:\Tomcat\apache-tomcat-7.0.30-windows-x86\apache-tomcat-7.0.30\webapps
