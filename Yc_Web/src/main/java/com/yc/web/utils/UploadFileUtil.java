...
public class UploadFileUtil {
	...
	public static Map<String, UploadFile> uploadFile(HttpServletRequest request, List<MultipartFile> files, String picRootName ) {
		...
		if (files != null && files.size() > 0) {
			//request.getSession().getServletContext().getRealPath("/")=> c：tomcat\apache-tomcat-7.0.47\webapps\douban
			//getParentFile()=> c:\tomcat\apache-tomcat-7..0.47\webapps
			File webappsfile=new File( request.getSession().getServletContext().getRealPath(  "/"     )).getParentFile();
			//图片保存的服务器位置   => c:\\tomcat\\apache-tomcat-7.0.47\\webapps\\uploadPdfs*/
			...
			for (MultipartFile multipartFile : files) {
				try {
					...
					//c:\\tomcat\\apache-tomcat-7.0.47\\webapps\\uploadPdfs\\/2017/2/20/
					String saveDir=picFile.getAbsolutePath()+getNowDateStr();
					//c:\\tomcat\\apache-tomcat-7.0.47\\webapps\\uploadPdfs\\/2017/2/20/ xxx.pdf
